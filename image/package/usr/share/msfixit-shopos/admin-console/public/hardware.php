...
:root{font-family:Inter,system-ui,sans-serif;color:#172033;background:#f4f7fb;--brand:#6a2ca0;--ok:#197447;--warn:#a45b00;--bad:#b4233a}*{box-sizing:border-box}body{margin:0}.shell{max-width:1200px;margin:auto;padding:24px}.top{display:flex;justify-content:space-between;gap:18px;align-items:start}.back{color:#536071;text-decoration:none}.grid{display:grid;grid-template-columns:repeat(12,minmax(0,1fr));gap:16px;margin-top:18px}.card{grid-column:span 4;background:white;border:1px solid #e1e7ee;border-radius:20px;padding:20px;box-shadow:0 8px 28px #1720330a}.wide{grid-column:span 6}.full{grid-column:1/-1}.metric{font-size:32px;font-weight:850}.muted{color:#687684}.pill{display:inline-flex;padding:6px 10px;border-radius:999px;background:#edf0f3;font-weight:800;font-size:12px}.warning{background:#fff0dc;color:var(--warn)}.critical,.emergency{background:#fde8ec;color:var(--bad)}.notice{padding:13px;border-radius:12px;margin:14px 0}.success{background:#e4f5eb;color:var(--ok)}.error{background:#fde8ec;color:var(--bad)}table{width:100%;border-collapse:collapse}td,th{padding:9px;border-bottom:1px solid #edf1f4;text-align:left}form{display:grid;gap:10px;margin-top:12px}input,select,button{font:inherit;padding:10px;border-radius:10px;border:1px solid #ccd5df}button{border:0;background:var(--brand);color:white;font-weight:800}.rec{padding:14px 0;border-bottom:1px solid #edf1f4}.rec:last-child{border:0}@media(max-width:850px){.card,.wide{grid-column:1/-1}.top{display:block}}</style></head><body><main class="shell">
...
<article class="card wide"><h2>Speicher & Peripherie</h2><p><strong><?=e((string)($storage['boot_medium']??'unbekannt'))?></strong> · <?=e(bytes(isset($storage['free_bytes'])?(int)$storage['free_bytes']:null))?> frei</p><p><?=count($usb)?> USB-Geräte · <?=count($printers)?> Drucker erkannt</p><?php foreach($printers as $p):?><span class="pill"><?=e((string)($p['name']??''))?>: <?=e((string)($p['state']??''))?></span> <?php endforeach;?></article>
