...
class HardwareManager:
    def __init__(
        self,
        *,
        state_dir: str = "/var/lib/msfixit-shopos/hardware-manager",
        socket_path: str = "/run/msfixit-shopos/hardware-manager.sock",
    ) -> None:
    ...
    def _primary_temperature(self, sensors: list[ThermalSensor]) -> float | None:
        if not sensors:
            return None
        preferred: list[str]
        if self.platform.platform_family == "raspberry-pi":
            preferred = ["cpu-thermal", "cpu_thermal", "soc_thermal", "soc"]
        else:
            preferred = ["package", "cpu", "coretemp", "tctl", "tdie", "soc"]
    ...
    def _thermal_snapshot(self, sensors: list[ThermalSensor]) -> ThermalSnapshot:
        ...
        if decision.changed:
            ...
            self.store.add_event(
                "thermal-state",
                severity,
                f"Temperaturstufe: {decision.previous_level} → {decision.level}",
        ...
        if decision.shutdown_eligible:
            # This first release intentionally records the decision but does not
            # power off by itself until the hardware path has been validated on
            # real devices. The decision path is fully testable and fail-closed.
            self.store.add_event(
                "emergency-shutdown-eligible",
                "critical",
                "Notfalltemperatur ist lang genug bestätigt; geregeltes Herunterfahren wäre zulässig.",
                temperature_c=primary,
                emergency_seconds=decision.emergency_seconds,
            )
            LOG.critical("emergency shutdown eligible after %ss at %s C", decision.emergency_seconds, primary)
    ...
    def sample(self) -> Snapshot:
        with self._lock:
            ...
            sensors = self.collector.thermal_sensors()
            thermal = self._thermal_snapshot(sensors)
            storage = self.collector.storage()
            network = self.collector.network()
            usb = self.collector.usb()
            services = self.collector.services()
            recommendations = self.rules.evaluate(
                platform_family=self.platform.platform_family,
                cpu=cpu,
                memory=memory,
                thermal=thermal,
                storage=storage,
    ...
    def _apply_automatic_recommendations(self, snapshot: Snapshot, now: float) -> None:
        ...
        for recommendation in snapshot.recommendations:
            ...
            self.store.add_event(
                ...
                severity,
                f"Automatische Optimierung {recommendation.id}: {'erfolgreich' if result.get('ok') else 'fehlgeschlagen'}.",
                transaction_id=result.get("transaction_id"),
                error=result.get("error"),
            )
            LOG.info("automatic optimization %s result=%s", recommendation.id, result)

    def _require_password(self, request: dict[str, Any]) -> bool:
    ...
    def api_request(self, request: dict[str, Any]) -> dict[str, Any]:
        ...
        with self._lock:
            ...
            if method == "GET" and path == "/recommendations":
                values = [asdict(item) for item in (self.store.latest.recommendations if self.store.latest else [])]
                return {"ok": True, "recommendations": values}
            ...
            if method == "GET" and path == "/diagnostic":
                return {
                    "ok": True,
                    "diagnostic": {
                        "snapshot": self.store.latest.as_dict() if self.store.latest else None,
                        "events": list(self.store.events),
                        "settings": asdict(self.store.settings),
                        "note": "Keine Passwörter, Tokens, IP- oder MAC-Adressen werden vom Hardware-Manager erfasst.",
                    },
                }
            ...
            if path == "/actions/apply":
                ...
                self.store.add_event(
                    ...
                    f"Optimierung {recommendation.id}: {'erfolgreich' if result.get('ok') else 'fehlgeschlagen'}.",
                    transaction_id=result.get("transaction_id"),
                    error=result.get("error"),
                )
                return result
            ...
            if path == "/actions/rollback":
                ...
                self.store.add_event(
                    "rollback",
                    "info" if result.get("ok") else "warning",
                    f"Rollback {transaction_id}: {'erfolgreich' if result.get('ok') else 'fehlgeschlagen'}.",
                    error=result.get("error"),
                )
                return result
    ...
    def start_api(self) -> threading.Thread:
        ...
        try:
            ...
            gid = os.getgid()  # type: ignore[name-defined]
