...
for f in "${shell_files[@]}"; do
    ...
    if [[ "$first" == *'/bin/sh'* && "$first" != *bash* ]]; then
        run_error "POSIX shell syntax: $f" sh -n "$f"
    else
        run_error "Bash syntax: $f" bash -n "$f"
    fi
    command -v shellcheck >/dev/null 2>&1 && run_warning "ShellCheck warning-or-higher: $f" shellcheck -x -S warning "$f"
done

mapfile -t py_files < <(
  { git ls-files '*.py'; git ls-files 'image/package/usr/local/bin/*' 'image/package/usr/local/sbin/*' 'scripts/*'; } \
  | sort -u | while IFS= read -r f; do
      [ -f "$f" ] || continue
      first="$(head -n1 "$f" 2>/dev/null || true)"
      case "$f:$first" in *.py:*|*:*python*) printf '%s\n' "$f" ;; esac
    done
)
for f in "${py_files[@]}"; do
    checked=$((checked+1))
    run_error "Python compile: $f" python3 -m py_compile "$f"
done
...
if command -v php >/dev/null 2>&1; then
    for f in "${php_files[@]}"; do
        checked=$((checked+1))
        run_error "PHP lint: $f" php -l "$f"
    done
else
    warn 'php-cli unavailable'
fi

while IFS= read -r f; do
    [ -f "$f" ] || continue
    checked=$((checked+1))
    run_error "Invalid JSON: $f" jq empty "$f"
done < <(git ls-files '*.json')

mapfile -t yaml_files < <(git ls-files '*.yml' '*.yaml')
if (( ${#yaml_files[@]} )); then
    run_error 'Invalid YAML' ruby -e 'require "yaml"; ARGV.each{|f| YAML.parse_file(f)}' "${yaml_files[@]}"
fi
...
[ -z "$sudo_wild" ] || { printf '%s\n' "$sudo_wild" >> "$report"; warn 'Wildcard found in packaged NOPASSWD sudoers command'; }
...
php_exec="$(git grep -nE '(shell_exec|exec|system|passthru|proc_open|popen|pcntl_exec)[[:space:]]*\(' -- '*.php' 2>/dev/null || true)"
[ -z "$php_exec" ] || { printf '%s\n' "$php_exec" >> "$report"; warn 'PHP process execution primitive requires manual allowlist/shell-boundary review'; }
shell_eval="$(git grep -nE '(^|[;[:space:]])eval[[:space:]]+' -- 'image/package/**' 'scripts/**' ':!scripts/audit-full-repo-v2.sh' 2>/dev/null || true)"
...
while IFS= read -r unit; do
    ...
    if grep -Eq '^Restart=always$' "$unit" && ! grep -Eq '^StartLimitIntervalSec=|^StartLimitBurst=' "$unit"; then
        warn "$unit restarts forever without explicit rate limit"
    fi
    if grep -Eq '^Exec(Start|StartPre|StartPost)=.*(/bin/(ba)?sh|/usr/bin/(ba)?sh)[[:space:]]+-c' "$unit"; then
        warn "$unit launches a shell -c from systemd; verify arguments cannot contain untrusted data"
    fi
...
while IFS= read -r test_file; do
    ...
    if [ -z "$refs" ]; then
        warn "Regression test is not referenced by Makefile/workflows/another test: $test_file"
    fi
...
if [ -f "$nginx" ]; then
  ...
  if [ -n "$generic" ] && [ -n "$deny" ] && [ "$generic" -lt "$deny" ]; then
      error 'Nginx generic PHP handler precedes writable-upload PHP deny'
  fi
fi

if [ -f "$firstboot" ] && [ -f "$firstlogin" ]; then
  if grep -Fq 'OS_ADMIN_PASSWORD' "$firstboot" || grep -Eq '(^|[[:space:]])chpasswd([[:space:]]|$)' "$firstboot"; then
      error 'Firstboot races interactive first-login for Linux administrator credentials'
  fi
fi
...
if [ -f "$cloud_service" ]; then
  if grep -Eq 'ExecStart=.*--token[[:space:]]+\$\{?CF_TUNNEL_TOKEN|EnvironmentFile=.*cloudflared.*env' "$cloud_service"; then
      error 'Cloudflare tunnel secret can reach the process environment/argument vector instead of token-file'
  fi
  grep -Fq -- '--token-file' "$cloud_service" || warn 'Cloudflare service does not use --token-file'
fi

if [ -f "$backup" ] && [ -f "$admin" ]; then
  if grep -Fq '.tar.gz' "$backup" && grep -Fq 'shopos-*.tar.zst' "$admin"; then
      error 'Admin dashboard backup glob does not match backup writer extension'
  fi
  if grep -Fq '.tar.zst' "$backup" && grep -Fq 'shopos-*.tar.gz' "$admin"; then
      error 'Admin dashboard backup glob does not match backup writer extension'
  fi
fi

if [ -f "$updates" ] && grep -Fq 'function helper(' "$updates" && grep -Fq '$timeout' "$updates" && ! grep -Eq 'stream_select|proc_get_status|hrtime|microtime' "$updates"; then
  warn 'Admin update helper accepts a timeout but does not visibly enforce it'
fi

if [ -f "$build_workflow" ] && grep -Fq 'gh release create' "$build_workflow"; then
  error 'Image build workflow can publish a stable GitHub Release before the dedicated system release gate'
fi

if [ -f "$firstboot" ] && grep -Fq 'msfixit-shopos.local' "$firstboot" && ! grep -Eq '^Depends:.*(avahi-daemon|libnss-mdns)' "$control"; then
  warn '.local fallback is advertised without explicit mDNS runtime dependency'
fi

if [ -f scripts/build-package.sh ] && grep -Fq 'BUILD_UTC=%s' scripts/build-package.sh && ! grep -Fq 'SOURCE_DATE_EPOCH' scripts/build-package.sh; then
  warn 'Build embeds wall-clock time without SOURCE_DATE_EPOCH support'
fi

if [ -f scripts/fetch-vendor-assets.sh ] && grep -Fq 'downloads.wordpress.org/plugin/woocommerce' scripts/fetch-vendor-assets.sh \
   && ! grep -Eq 'woocommerce_(sha256|SHA256)|WOOCOMMERCE_SHA256' scripts/fetch-vendor-assets.sh scripts/build-package.sh; then
  warn 'WooCommerce vendor artifact lacks a pre-pinned expected SHA-256'
fi

if [ -f "$update_agent" ]; then
  grep -Eq 'flock|fcntl.*LOCK' "$update_agent" || warn 'A/B update agent has no visible exclusive writer lock'
  if grep -Fq 'issued_at' "$update_agent" && grep -Fq 'expires_at' "$update_agent" && ! grep -Eq 'fromisoformat|date[[:space:]].*-d|datetime' "$update_agent"; then
      warn 'Update manifest timestamps are present but no obvious temporal parsing is visible in update agent'
  fi
fi
