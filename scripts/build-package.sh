...
# Normalize every packaged filesystem timestamp to the source commit epoch.
# dpkg-deb also honors SOURCE_DATE_EPOCH, so identical source + pinned inputs
# produce identical archive metadata instead of embedding runner wall-clock time.
