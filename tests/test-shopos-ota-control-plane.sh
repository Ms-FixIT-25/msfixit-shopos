...
p=pathlib.Path(__import__('sys').argv[1]); spec=importlib.util.spec_from_file_location('ota',p); m=importlib.util.module_from_spec(spec); spec.loader.exec_module(m)
with tempfile.TemporaryDirectory() as d:
    db=f'{d}/ota.sqlite3'; m.init_db(db)
    with m.connect(db) as c:
        tables={r[0] for r in c.execute("SELECT name FROM sqlite_master WHERE type='table'")}
        assert {'devices','notifications'} <= tables
    good={'schema':1,'device_id':'d','client_instance':'i','architecture':'aarch64','shopos_version':'1','channel':'stable'}
    m.validate_registration(good)
    try: m.validate_registration({**good,'channel':'evil'})
    except ValueError: pass
    else: raise AssertionError('invalid channel accepted')
    token='x'*40
    assert m.token_hash(token) != token
print('PASS: OTA control-plane schema and registration guards')
PY
