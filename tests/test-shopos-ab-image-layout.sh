...
with tempfile.TemporaryDirectory() as tmp:
    ...
    cmdline.write_text('console=serial0,115200 consoleblank=60 root=PARTUUID=deadbeef-02 rootfstype=ext4 rw quiet consoleblank=300\n')
    ...
    assert 'consoleblank=60' not in value
    assert 'consoleblank=300' not in value
    module.select('A', cmdline)
    value = cmdline.read_text()
    assert 'root=LABEL=SHOPOS_ROOT_A' in value
    assert value.split().count('consoleblank=0') == 1
print('PASS: A/B slot selection preserves exactly one consoleblank=0 policy across updates.')
PY
