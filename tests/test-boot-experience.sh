...
bash -n image/package/usr/local/sbin/msfixit-x-session
python3 -m py_compile image/package/usr/local/sbin/msfixit-setup-gui
...
for file in \
    ...
    image/package/etc/systemd/system/msfixit-kiosk.service \
    image/package/usr/local/sbin/msfixit-x-session \
    image/package/usr/local/sbin/msfixit-setup-gui \
...
# The text boot console remains available only as a diagnostic fallback. Its
# renderer must stay stable, but production boot must not auto-start it.
...
assert_contains 'systemctl disable msfixit-boot-console.service' image/package/DEBIAN/postinst
assert_contains 'systemctl disable msfixit-first-login.service' image/package/DEBIAN/postinst
assert_contains 'systemctl enable msfixit-kiosk.service' image/package/DEBIAN/postinst
assert_contains 'plymouth, plymouth-themes, initramfs-tools' image/package/DEBIAN/control

# Plymouth hands off to one persistent local Xorg server. The first-login GUI
# and Chromium are clients of that same server; no legacy tty wizard is allowed
# to become the production display owner again.
assert_contains 'ExecStartPre=-/usr/bin/plymouth quit --retain-splash' image/package/etc/systemd/system/msfixit-kiosk.service
assert_contains 'ExecStart=/usr/bin/xinit /usr/local/sbin/msfixit-x-session -- :0 vt7 -keeptty -nolisten tcp' image/package/etc/systemd/system/msfixit-kiosk.service
assert_contains 'ConditionPathExists=/usr/local/sbin/msfixit-setup-gui' image/package/etc/systemd/system/msfixit-kiosk.service
assert_contains '/usr/local/sbin/msfixit-setup-gui' image/package/usr/local/sbin/msfixit-x-session
...
assert_contains 'xset -dpms' image/package/usr/local/sbin/msfixit-x-session
assert_contains 'xset s off' image/package/usr/local/sbin/msfixit-x-session
assert_contains 'xset s noblank' image/package/usr/local/sbin/msfixit-x-session
...
echo 'ShopOS Plymouth-to-persistent-X boot experience and offline payload checks passed.'
