...
The optimized Ms. FixIT artwork is stored as repository-safe base64 source text. During package construction it is decoded to WebP, checked against its fixed SHA-256 value, and the base64 source is removed from the final package. PHP GD generates the header and site-icon variants during branded shop provisioning.
